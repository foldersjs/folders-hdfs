 ...
 * 
 * @param uri,
 *          the uri on hdfs to ls eg, '/', '/sub-folder1', or dir start with {prefix},
 *          '/http_window.io_0:webhdfs/sub-folder1'
 * @param cb,
 *          callback function(err, result) function. result will be a file info array. [{}, ... {}], a example file
 *          information
 * 
 * <pre>
 * {
...
FoldersHdfs.prototype.ls = function(path, cb) {
  ...
  request(self.op(path, WebHdfsOp.LIST), function(err, response, content) {
    ...
    try {
      ...
      return cb({
        "errorMsg" : "parse result error in server"
      }, null);
    }
 ...
 *          string, the path
 * @param data,
 *          the input data, 'stream.Readable' or 'Buffer'
 * @param cb,
 *          the callback function write(path,data,cb)
 */
FoldersHdfs.prototype.write = function(uri, data, cb) {
  ...
  request.put(path, function(error, response, body) {
    ...
    if (!isRedirect(response)) {
      var errMsg = "expecting redirect 307, return un-expected status code, statusCode:" + response.statusCode;
    ...
    if (data instanceof Buffer) {
      ...
      data.on('error', errHandle).pipe(request.put(redirectedUri).on('error', errHandle).on('end', function() {
 ...
 * 
 * @param uri,
 *          the file uri to cat or the param object { path: uri, offset: 0, length: 10 }
 * @param cb,
 *          callback function(err, result) function. example for result. { stream: .., // a readable 'request' stream
 *          size : .. , // file size name: path }
 */
FoldersHdfs.prototype.cat = function(data, cb) {
  ...
  if (typeof (data) === 'string') {
    ...
    if (data.offset)
      offsetParams += '&offset=' + data.offset;
    if (data.length)
      offsetParams += '&length=' + data.length;
  }
 ...
 * @param cb,
 *          callback function(err, result) function. example for result. {"boolean":true}
 */
    ...
    (function(i) {
      ...
      request(self.op(path + files[i].pathSuffix, WebHdfsOp.DIRECTORY_SUMMARY), function(err, response, statsResponse) {
        ...
        if (!results[i].meta)
          results[i].meta = {};
        for ( var meta in cols)
          results[i].meta[cols[meta]] = stats[cols[meta]];
        return latchDecrementAndCb();
